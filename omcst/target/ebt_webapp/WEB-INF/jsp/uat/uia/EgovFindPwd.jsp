...
function fnSearch()
{
	if($('#userId').val() == "")
	{
		alert('사용자 아이디는 필수 입력 항목입니다.');
		return false;
	}

	if($('#answer').val() == "")
	{
		alert('답변 항목은 필수 입력 항목입니다.');
		return false;
	}
	...
	$.ajax({
	        type: "post",
	        url : "${pageContext.request.contextPath}/uat/uia/selectEgovFindPwd.do",
	        data : obj,
	        success : function (data){
	        	console.log(data);
	        	if(data == '1')
	        	{
	        		if(confirm( $('#userId').val()+"님의 패스워드를 초기화 하시겠습니까?"))
	        		{
	        			let obj={
        					"userId" : $('#userId').val()
        				};
        				...
        				$.ajax({
       				        type: "post",
       				        url : "/uat/uia/updateEgovFindPwd.do",
       				        data : obj,
       				        success : function (data){
      				        		if(data == '1')
      				        		{
      				        			alert('사용자 패스워들  초기화 하였습니다.');
      				        		}
      				        		else
      				        		{
      				        			alert('사용자 패스워 변경에 실패하였습니다.');
      				        		}
       				        }
        				})
	        		}
            ...
            <div class="pop_container">
                <div class="board_view2">
                    <table>
                        ...
                        <tr>
                            <td class="lb">
                                <span class="min">사용자 아이디</span>
                                <span class="req">필수</span>
                            </td>
                            <td>
                             	<input class="f_input w_500" id="userId" type="text" maxlength="35" title="아이디">
                            </td>
                        </tr>
                        <tr>
	                       	<td class="lb" >
	                           ...
	                           <label class="f_select w_full" for="passwordHint">
			                         <select name="hint" id="hint" title="패스워드 질문">
			                             	<c:forEach var="pwdCode" items="${passwordHint}" varStatus="status">
			                                 	<option value="<c:out value="${pwdCode.code}"/>"><c:out value="${pwdCode.codeNm}"/></option>
			                                 </c:forEach>
			                         </select>
	                       		</label>
	                         </td>
                        ...
                        <tr>
                            <td class="lb">
                                <span class="min">답변</span>
                                <span class="req">필수</span>
                            </td>
                            <td>
                             	<input class="f_input w_500" id="answer" type="text" maxlength="35" title="아이디">
                            </td>
                        </tr>
                    </table>
                </div>
