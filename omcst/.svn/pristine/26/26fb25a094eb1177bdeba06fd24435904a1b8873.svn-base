...
public class EgovBBSManageServiceImpl extends EgovAbstractServiceImpl implements EgovBBSManageService {
	...
	// SORT_ORDR는 부모글의 소트 오더와 같게, NTT_NO는 순서대로 부여
		...
		if ("Y".equals(board.getReplyAt())) {
		   ...
		   // tmpNttId = bbsMngDAO.replyBoardArticle(board);
		    Board bo = bbsMngDAO.replyBoardArticle(board);
		    return bo;
		    ...
		    Board bo = bbsMngDAO.insertBoardArticle(board);
    ...
    public Map<String, Object> selectBoardArticles(BoardVO boardVO, String attrbFlag) throws Exception {
    	...
    	switch(attrbFlag) {
	    	case "BBSA02":
	    		result = bbsMngDAO.selectBoardGalleryList(boardVO);
		    	cnt = bbsMngDAO.selectBoardGalleryListCnt(boardVO);
		    	break;
	    	...
	    	case "BBSA01":
			    ...
			    cnt = bbsMngDAO.selectBoardArticleListCnt(boardVO);
	    		break;
    		
	    	case "BBSA03":
	    		// 공지사항의 첫페이지인 경우 고정 공지사항 조회
	    		if(boardVO.getBbsId().equals("BBSMSTR_AAAAAAAAAAAA") && boardVO.getFirstIndex() == 0) {
	    			map.put("fixedList", bbsMngDAO.selectFixedArticleList(boardVO));
	    		}
    			result = bbsMngDAO.selectBoardArticleList(boardVO);
    			cnt = bbsMngDAO.selectBoardArticleListCnt(boardVO);
    			break;
